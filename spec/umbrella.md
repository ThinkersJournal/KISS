...
The suite-wide role words — **consumer**, **provider**, **producer** / **emitter**, and **implementation** — name what a party *does to a KISS surface*, not a fixed identity a party carries. One party is routinely a consumer of one surface and a producer of another. Treating a role as a blanket identity is the defect RFC #500 records: a clause whose quantifier ranges over a broader set (every party) than the obligation it states (every party *acting on the surface the clause is about*). These definitions are canonical for the suite; where a sub-standard's §3 Terms entry names one of these words, it states the same role scoped to that sub-standard's surface, and may add membership conditions within that surface. This section is the canonical statement where those wordings conflict; a sub-standard condition that narrows the role is a refinement, not a conflict. Which of act, capability or claim makes a party a member of a role on a surface is not decided here; each re-scope states it.
...
2. **wire** — every KISS byte artifact, whichever sub-standard owns it: not only the KISS-Announce / KISS-Classify / KISS-Contract artifacts but also KISS-Ops's own wire encodings — its OpAttrs channel (KISS-Ops §6.19) and its shape expressions (KISS-Ops §6.20) — **parse** and/or **produce**. This surface is **not atomic**: a party may produce one wire artifact and parse another (a party may produce the sk4 `structure_key` while parsing the KISS-Ops §6.20 shape-expression encoding), so a clause binding it names the artifact (see below).
...
- **producer / emitter of *S*** — a party that produces *S* (an emitter is a producer of {artifact, contract}; a lifter is a producer of {Semantics field, recorded residue} on op-semantics).
