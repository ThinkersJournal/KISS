...
- **Editor–implementer identity (bounded).** The editor is also a conformant
  implementation of both directions. Two constraints hold in consequence: the editor's
...
precondition). It is *a* conformant implementation with no privilege; the reference
implementation runs the same public KISS-Conform suite with no exemption. Project and
