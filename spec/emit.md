...
`structure_key` for the cell `sk2|bin|f32|cuda:sm89|ix32|grid|r2|…` (three `f32`
...
- **KISS-EMIT-6.4-0004** — **Target-specific type spelling**, and the surface form of
  any dtype that differs by target language (`f16`, `bf16`, the FP8 formats `e4m3` /
...
Input `(OpDef=add, structure_key=sk2|bin|f32|cuda:sm89|ix32|grid|r2|…)`. The complete
