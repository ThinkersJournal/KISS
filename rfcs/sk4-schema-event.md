...
1. **Accumulator (numeric).** A mixed-precision non-contraction reduction (`f16` data, `f32`
   ...
   an sk3-un-discriminable non-contraction `f32` reduction (provider side, `contract.rs:1822`).
...
(`fuel-ir dtype.rs:102`), adds the `fn` **suffix only**. The target is `f8e4m3fn`/`f8e5m2` regardless
...
`to_token()` output (`unpopped/src/contract.rs:746`) — so on spelling the corpus is **downstream of
...
- **Motivation strengthening.** `kiss-classify-vocab/src/lib.rs:42` documents the `s8/s16`-vs-`i32/i64`
