//! KISS-CONFORM-6.5-0011 — the oracle-vector envelope. Required fields, a typed decline on an
...
fn test_conform_oracle_vector_envelope() {
    ...
    let a = corpus::load(&arith()).expect("ops-arith.json validates under §6.5-0011");
    assert_eq!(a.schema, "kiss-oracle-vectors-v1.json");
    assert_eq!(a.schema_version, 1);
    let m = corpus::load(&minmax()).expect("ops-minmax-signed-zero.json validates under §6.5-0011");
    ...
    assert!(
        ...
        "§6.5-0011: an unrecognized `schema` must decline, not load as if v1"
    );
    ...
    assert!(
        ...
        "§6.5-0011: an unrecognized `schema_version` must decline, not load as if v1"
    );
    ...
    assert!(
        ...
        "§6.5-0011: a bundle missing `schema_version` must decline"
    );
    ...
    // --- The invariance §6.5-0011 claims explicitly: every op in the two committed files falls to
